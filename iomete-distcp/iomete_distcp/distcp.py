...
class DistCp:
    ...
    def run(self, source_path: str, dest_path: str) -> None:
        ...
        def copy_partition(iterator: Any) -> List[Dict[str, Any]]:
            results = []
            for row in iterator:
                result = DistCp.copy_file(
                    row["source_uri"], row["dest_uri"], row["path"]
                )
        ...
        self.logger.info(
            f"Copy completed: {successful} successful, {failed} failed, {total_size} bytes copied"
        )
        ...
        if failed > 0:
            ...
            for result in copy_results:
                if result["status"] == "failed":
                    self.logger.error(
                        f"  {result['source_path']}: {result.get('error', 'Unknown error')}"
                    )
...
def main() -> None:
    ...
    parser.add_argument(
        "source", help="Source path (s3://bucket/path, hdfs://host/path, etc.)"
    )
