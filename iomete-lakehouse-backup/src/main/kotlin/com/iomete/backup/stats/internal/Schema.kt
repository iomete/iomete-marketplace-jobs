...
internal val RUNS_SCHEMA: StructType =
    StructType(
        arrayOf(
            ...
            field("executor_count", INT, "executors the run was configured for", nullable = false),
            field("vcpu_per_executor", DOUBLE, "CPU limit of one executor pod", nullable = false),
            field(
                "slots_per_executor",
                INT,
                "concurrent copies per executor, as Spark ran them; divided by vcpu_per_executor gives copy.slotsPerVcpu",
                nullable = false,
            ),
            field("tasks_per_slot", INT, "copy.tasksPerSlot for this run", nullable = false),
            field("task_count", INT, "Spark tasks the copy was split into"),
            field("max_files_in_task", INT, "files in the fullest task; far above the average means small files cost more than charged"),
