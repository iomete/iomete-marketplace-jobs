...
def get_config(application_config_path) -> ApplicationConfig:
    ...
    if 'expire_snapshot' in config:
        app_config.expire_snapshot = ExpireSnapshotConfig(retain_last=config['expire_snapshot'].get('retain_last', 1))
    ...
    if 'gc_handling' in config:
        app_config.gc_handling = GCHandlingConfig(enabled=config['gc_handling'].get('enabled', False))
