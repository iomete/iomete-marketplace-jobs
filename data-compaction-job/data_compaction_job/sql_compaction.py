...
from data_compaction_job.stats_emitter import emit_stats, init_emitter
...
class SqlCompaction:
        ...
        with ThreadPoolExecutor(max_workers=self.config.parallelism) as executor:
            ...
            for database in databases:
                for table in db_table_mapping[database]:
                    futures.append(
                        executor.submit(
                            self.__process_table_if_iceberg,
                            catalog,
                            database,
                            table,
                        )
    ...
    def __set_gc_enabled(self, catalog, database, table_name, enabled):
        try:
            ...
            # Set the property
            self.spark.sql(
                f"ALTER TABLE {catalog}.{database}.{table_name} SET TBLPROPERTIES ('gc.enabled' = '{value}')"
            ).collect()
    ...
    def __expire_snapshots(self, catalog, database, table_name):
        ...
        retain_last = int(
            self.__get_final_config_for_table(database, table_name, 'expire_snapshot', 'retain_last')
    ...
    def __remove_orphan_files(self, catalog, database, table_name):
        days = int(
            self.__get_final_config_for_table(database, table_name, 'remove_orphan_files', 'older_than_days')
    ...
    def __rewrite_manifest(self, catalog, database, table_name):
        ...
        use_caching = (
            self.__get_final_config_for_table(database, table_name, 'rewrite_manifest', 'use_caching')
    ...
    def __rewrite_data_files(self, catalog, database, table_name):
        strategy = (
            self.__get_final_config_for_table(database, table_name, 'rewrite_data_files', 'strategy')
            or self.config.rewrite_data_files.strategy
        )
        sort_order = (
            self.__get_final_config_for_table(database, table_name, 'rewrite_data_files', 'sort_order')
            or self.config.rewrite_data_files.sort_order
        )
        rewrite_options = (
            self.__get_final_config_for_table(database, table_name, 'rewrite_data_files', 'options')
            or self.config.rewrite_data_files.options
        )
        where = (
            self.__get_final_config_for_table(database, table_name, 'rewrite_data_files', 'where')
    ...
    def __get_databases(self, catalog):
        available_databases = [
            database.namespace for database in self.spark.sql(f'show databases from {catalog}').collect()
        ]
    ...
    def __get_tables(self, catalog, database):
        available_tables = [
            table.tableName for table in self.spark.sql(f'show tables from {catalog}.{database}').collect()
        ]
        if database in self.__get_table_includes():
            tables = [table for table in self.__get_table_includes()[database] if table in available_tables]
        elif database in self.__get_table_excludes():
            tables = [table for table in available_tables if table not in self.__get_table_excludes()[database]]
        else:
...
class SqlClient:
    def __init__(self):
        ...
        self.base_url = os.getenv(
            'SQL_API_ENDPOINT',
            f'http://iom-core.{release_namespace}.svc.{cluster_domain}',
        )
